...
## 16-B. 2026-06 アルゴリズム大規模アップデート (サブエージェント10体並列リサーチ)

世界中の暦・天文ソースと突合した結果に基づく修正と新機能。

### ⚠ 最重要: 日柱の修正 (戊申 → 丙申)

旧実装の日干支アンカー (1900-01-01 = **丙**戌) が誤りで、全日柱の天干が +2 ズレていた。
正アンカー = 1900-01-01 = **甲**戌。外部突合: 2025-12-21 = 甲子 (陽遁始め)・
2026-03-05 = 戊寅 (天赦日)・2026-05-20 = 甲午・2026-10-01 = 戊申・2026-12-16 = 甲子 —
3系統の独立ソース群 + JDN 公式 (JDN+49) mod 60 すべてと一致。

- 本人 1984-05-02: 日柱 戊申 → **丙申**、時柱 己未 → **乙未** (丙日未時・五鼠遁)
- 日主が 戊(陽土) → **丙(陽火)** に変更。地支 (子辰申未) は不変
...
- 月 = **牡牛座 23.5°** (Meeus truncated 式 53.5°、外部天文データと 0.1° 以内で一致)。
  旧表記「射手座〜山羊座圏」は誤りだった (新月翌日で月は太陽の近く)
- ASC = **乙女座 1.5°** (13:00 ちょうど。12:54 以前なら獅子座 — カスプ注記を owner.ts に保持)
- MC = **牡牛座 28.1°** (太陽が MC 近傍の南中生まれ)

### 新機能: 暦 (`lib/koyomi.ts` + `app/koyomi/`)

- 開運日: 天赦日 (季節×日干支)・一粒万倍日 (節月×日支)・寅の日・巳の日・己巳・甲子。
  2026年の外部一覧と全件一致 (天赦日 6/6、一粒万倍 13/13 検証)
- 旧暦: 朔 (月-太陽の離角ゼロ交差を二分法) + 中気 (太陽黄経 30° 倍数) から天保暦方式で組み立て。
  検証: 2026 旧正月 = 2/17、2026-06-15 = 旧5/1。※ 2033年問題は未対応
- 六曜: (旧暦月+日) mod 6。検証: 2026-06-11 = 大安、2026-01-01 = 大安
- 不成就日: 旧暦月グループ × 8日周期
- 節月は暦注では「日」単位で切替 (節入り日全体が新月扱い)。四柱の月柱は従来通り時刻ベース

### 新機能: 九星 日盤・月盤・吉方位 (`lib/kyuseiBoard.ts` + 暦ページに統合)

- 日盤: 冬至/夏至に最も近い甲子で陽遁 (一白から順行) / 陰遁 (九紫から逆行) を切替。
  検証: 2025-12-21 陽遁一白・2026-06-11 二黒・2026-06-19 陰遁九紫。閏遁は未実装 (2030年代要再確認)
- 月盤: 年支グループ (子午卯酉=寅月八白 / 辰戌丑未=五黄 / 寅巳申亥=二黒) から節月ごとに逆行。
  検証: 2026年 2月=八白・5月=五黄・6月=四緑
- 盤の回座: 方位の星 = ((定位星 + 中宮 - 6) mod 9) + 1
- 凶殺: 五黄殺・暗剣殺・本命殺・本命的殺・破 (支の対冲方位)。
  吉方位 = 本命星と相生・比和の星 (七赤の吉星 = 一白・二黒・六白・八白) で凶殺なし

### 新機能: 動物占い (`lib/animal.ts` + `app/animal/`)

- 番号 = 生まれ日の六十干支番号。60キャラ対応表をリサーチで収集 (複数ソース突合、信頼度を付記)
- 本人 = 丙申 No.33「活動的な子守熊」(日柱修正の結果。旧 戊申なら No.45)

### リサーチ済み・未実装 (次の改善候補)

- 宿曜: 現行の本命宿 (觜宿) は旧暦方式と一致確認済み。正式な「月宿傍通暦」(旧暦月別起宿表) への
  置き換えで日々の宿の精度向上が可能 (起宿表は取得済み)
- 四柱の節入り: 月柱境界を固定日近似 → `astronomy.ts` の節気時刻 (分単位) に置換可能。
  1984-05-02 の月柱 戊辰は検証済みで影響なし。年柱は立春の瞬間切替が正式
- 時柱の夜子時 (23時台の日柱切替) は流派差 — オプション化が安全
