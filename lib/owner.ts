...
export const OWNER = {
  ...
  natal: {
    sun: "taurus",
    // ===== 月・ASC・MC (2026-06 天文計算で正式値に昇格) =====
    // 月: Meeus truncated 式で黄経 53.5° = 牡牛座 23.5°。
    //     astro-seek (1984-05-02 の月 = 牡牛座 21°31′@0h, +0.55°/h) と 0.1° 以内で一致。
    //     新月 (1984-05-01 牡牛座) の翌日でもあり整合。旧値「射手座〜山羊座圏」は誤りだった。
    moon: "taurus",
    moonDegree: 23.5,
    // ASC: 13:00 JST ちょうどなら乙女座 1.5°。獅子座/乙女座カスプぎりぎりで、
    //      出生が 12:54 以前なら獅子座。時刻の精度に依存するためカスプ注記を保持。
    asc: "virgo",
    ascDegree: 1.5,
    ascCuspNote: "獅子座/乙女座の境界 +1.5°。出生 12:54 以前なら獅子座 ASC。",
    mc: "taurus",
    mcDegree: 28.1,
    fourPillars: {
      // ⚠ 2026-06 の外部暦突合で日柱の修正を実施:
      //   旧値 日柱=戊申・時柱=己未 はエンジンのアンカーずれ (天干+2) による誤り。
      //   正: 1984-05-02 = 丙申日 (2025-12-21=甲子, 2026-03-05=戊寅 等の
      //       複数外部ソースと JDN 公式 (JDN+49) mod 60 の両方で確認)。
      //   時柱は丙日の未時 → 五鼠遁 (丙辛日は戊子起) で 乙未。
      //   日主が 戊(陽土) → 丙(陽火) に変わるため、戊を前提に執筆された
      //   解釈文 (synthesis/synthesisDeep 等) は要改訂 (docs/ALGORITHMS.md 参照)。
      year: "甲子",
      month: "戊辰",
      day: "丙申",
      hour: "乙未",
    ...
    fengshui: {
      // ⚠ 本命卦の二重真実に注意:
      //   標準算式 calcKua(1984, 5, 2, "male") = (100-84) mod 9 = 7 (兌・西四命)
      //   本サイトの確定値 = 6 (乾・西四命)
      // サイト全体のコンテンツ (FENGSHUI_HOME / FENGSHUI_KUA6_DEEP /
      // FENGSHUI_ROOM_BY_ROOM / compat-rules.json) は 乾 6 を前提に執筆済み。
      // 幸い 6 も 7 も同じ西四命のため吉凶グループは一致するが、
      // 8 方位の細目 (生気/天医/延年/伏位 の配置) は異なる。
      // → この値を 7 に変更する場合は上記コンテンツの方位記述も全て要改訂。
      // → 流派差 (時刻補正・九星ベース等) で 6 を採用しているなら本人確認の上この注記を更新すること。
      kua: 6,
    },
    numerology: {
      lifePath: 11,
  ...
  perfumes: [
    ...
    { id: "dpt-tam-dao", brand: "Diptyque", name: "タム ダオ", family: "ウッディ・サンダル", notes: ["サンダルウッド", "シダー", "ローズウッド", "アンバー"], mood: ["寺院", "瞑想", "アジア"], season: ["autumn", "winter"], time: ["afternoon", "evening", "night"], weather: ["cold", "fog", "cloudy"], energyDays: [2, 7], description: "ベトナムの寺院の白檀。瞑想・神社参拝・丙火の昂りを鎮め本質を引き出す。" },
    ...
    { id: "lelabo-santal-33", brand: "Le Labo", name: "サンタル 33", family: "ウッディ", notes: ["サンダルウッド", "アイリス", "バイオレット", "カルダモン", "アンバー"], mood: ["落ち着き", "静寂", "知性"], season: ["autumn", "winter", "spring"], time: ["afternoon", "evening"], weather: ["cloudy", "cold", "fog"], energyDays: [2, 7], description: "白檀の静寂と煙の哲学。一人時間・読書・深い対話に。燃えやすい丙火を鎮める木の香りと好相性。" },
    ...
    { id: "tf-tobacco-vanille", brand: "Tom Ford", name: "タバコ バニラ", family: "オリエンタル・スパイシー", notes: ["タバコリーフ", "バニラ", "カカオ", "ドライフルーツ", "トンカビーン"], mood: ["重厚", "セクシー", "リッチ"], season: ["autumn", "winter"], time: ["evening", "night"], weather: ["cold", "snow", "rainy", "cloudy"], energyDays: [4, 8], description: "深く甘いタバコの煙とバニラ。重要な決断・夜の集まり・寒い夜に。丙火の温かみと相性◎。" },
