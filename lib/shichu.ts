...
export const TONGBIAN_VARIANTS: Record<TongbianStar, string[]> = {
  比肩: [
    ...
    "「自分は自分」という確固たる軸の星。誰かに従うより、自分でリスクを取り、自分で結果を受け取ることに快感を感じる。同年代の仲間と切磋琢磨することで力を発揮し、年功序列より実力本位の環境で輝く。",
  ],
  劫財: [
    "競争と協力の星。仲間と切磋琢磨し、ライバルとともに伸びるエネルギー。財を奪い合うように見えるが、実は競争があるからこそ実力が磨かれる。やや攻撃的・賭け事好きの傾向も。",
    "「奪う・奪われる」の力学が働く星。良くも悪くも金銭が大きく動き、稼ぐ時は大きく、出ていく時も大きい。チーム戦・パートナーシップで真価を発揮するが、油断すると裏切りや借金の場面に巻き込まれやすい。",
  ],
  食神: [
    "創造と楽しみの星。表現力・芸術・グルメ・遊び心を司る吉星。穏やかで人生を楽しむ才能、衣食住に困らない安定運の象徴。柔らかな魅力で人を惹きつける。",
    "人生を「美味しく」生きる才能の星。料理・音楽・絵画・文章 — 何かを作って人に喜んでもらうことに、自然な才能と幸福を見出す。焦らず、ゆったりと人生を味わう包容力が、最大の魅力。",
  ],
  傷官: [
    "鋭い才能の星。批評眼・独自性・反骨精神・芸術的センスを表す。常識や権威に楯突く傾向があり、組織内では衝突しやすいが、専門家・クリエイター・批評家として大きく花開く。",
    "「本物しか受け付けない」鋭い知性と感受性の星。中途半端な権威・伝統・常識を撃ち抜く目を持ち、独自の表現で世界を変える可能性を秘める。言葉のキレが武器にも凶器にもなる両刃の剣。",
  ],
  偏財: [
    "流通する財の星。社交と機転で稼ぐエネルギー。固定収入よりビジネス・副業・投資で動く財。男性命では恋愛運や父との縁を表すことも。気前が良く、お金を循環させる才。",
    "「お金を流す」才能の星。貯め込むより回すことで増えるタイプで、ビジネス・副業・投資・トレードに向く。人脈の広さがそのまま財運になる、社交家・営業マン・実業家の星。",
  ],
  正財: [
    "安定した財の星。コツコツ蓄える堅実な財運、家庭運、配偶者運（男性命では妻との縁）。地味だが確実に資産を積み上げる。投機より預金、派手より堅実。",
    "「育てる」財の星。給与・家賃・配当・地道な事業 — 長期で見れば最も確実に資産を築くタイプ。家庭・結婚・住宅といった「安定の象徴」を大切にし、その地盤の上に成功が積み上がる。",
  ],
  偏官: [
    "胆力と決断の星（七殺とも）。困難に立ち向かう強さ、リーダーシップ、武の才。攻撃性と勇気が紙一重で、活かせば英雄、暴走すれば破壊。改革者・軍人・経営者に多い配置。",
    "「修羅場で輝く」星。逆境ほど力を発揮し、平和な日常では物足りなさを感じる体質。決断の早さ・度胸・リーダーシップで人を引っ張るが、休まずに走り続けると燃え尽きるリスクも。",
  ],
  正官: [
    "規律と名誉の星。組織での昇進・社会的地位・公的評価を表す吉星。真面目で品行方正、責任感が強い。女性命では夫との縁、男性命では息子との縁、社会的責任の象徴。",
    "「ルールを愛する」星。秩序・公正・伝統に従い、その枠の中で頂点を目指すタイプ。公務員・大企業・士業で力を発揮し、長期的に信頼を積み上げて社会的地位を確立する。",
  ],
  偏印: [
    "独自の知性の星。アイデア・直感・副業・特殊技能を司る。表の主流から外れた知性、研究者・占術家・発明家・フリーランスの星。義母との縁を表すことも。",
    "「裏のルートで才能を見せる」星。王道ではなくニッチで光り、人と違う角度から物事を見る才能を持つ。占術・哲学・神秘学・芸術・テクノロジーの最先端で活躍する人に多い配置。",
  ],
  印綬: [
    "学問と保護の星。教養・教育・名誉・親（特に母）との縁・宗教的庇護を表す吉星。学問を通じた成功、年長者から愛される徳、人格的な深みの象徴。",
    "「学ぶことが運命」の星。本・学問・師との出会いが人生を作り、年長者・先輩・恩師からの引き上げが多い。知性と品格を兼ね備え、人生の後半になるほど深みと尊敬を獲得していくタイプ。",
  ],
};

export const TONGBIAN_TEXT: Record<TongbianStar, string> = Object.fromEntries(
  Object.entries(TONGBIAN_VARIANTS).map(([k, v]) => [k as TongbianStar, v[0]])
) as Record<TongbianStar, string>;

export function tongbianText(star: TongbianStar, date: Date = new Date()): string {
  const variants = TONGBIAN_VARIANTS[star];
  if (!variants) return TONGBIAN_TEXT[star] ?? "";
...
export const TWELVE_VARIANTS: Record<TwelveStage, string[]> = {
  長生: [
    "誕生・育成の段階。新しい命が芽吹くフレッシュなエネルギー。柔らかく素直で、周囲から愛されながら育っていく時期。学びと成長に追い風で、年長者からの引き上げが多い。",
    "「無垢な可能性」の段階。まだ何者でもないからこそ、何者にでもなれる時期。素直さと吸収力が最大の武器で、教えてくれる人を見つけることが運命を開く鍵になる。",
  ],
  沐浴: [
    "産湯を浴びる段階。揺らぎ・迷い・探索の時期。色気と移り気が同居し、恋愛運には変化が多い。試行錯誤が多い分、本質を掴む経験ができる。",
    "「揺らぎの中で本性が露わになる」段階。安定を求めるより、まずいろいろ試してみるエネルギー。一見遠回りに見える経験が、後に大きな財産になる試行錯誤のフェーズ。",
  ],
  冠帯: [
    "成人式の段階。実力をまとい、社会的な役割を担い始める時。意気軒昂で野心的、外見にも気を使い、自分のスタイルを確立する。挑戦と背伸びが似合う段階。",
    "「世に出る準備が整う」段階。教育期間が終わり、いよいよ社会の中で自分を試す時期。多少の背伸びと虚勢が、むしろ自分を大きく見せる装備となる勢いの段階。",
  ],
  建禄: [
    "自立の段階。自分の城・場・職を築く充実期。実力と地位が安定し、誰かに依存せず生きていける自立した力。家庭を持つ・独立するに最適なエネルギー。",
    "「自分の足で立つ」段階。組織の中でも、独立しても、自分の責任で自分の道を進めるだけの実力が備わる時期。結婚・起業・住宅購入など、人生の重要な礎が築かれる。",
  ],
  帝旺: [
    "頂点の段階。十二運で最も強い気を持ち、最大の力を発揮する時。リーダーシップ・成功・名声がピークに達する。同時に独走・傲慢の危険もあり、自制が運命を分ける。",
    "「王の段階」。生まれ持ったエネルギーが頂点を極め、人を率い、世界に影響を及ぼす立場になる。同時に「頂点の次は下り坂」という宇宙の法則も常に背中で意識すべき時期。",
  ],
  衰: [
    "勢いが緩み始める転換点。頂点を過ぎて落ち着きが訪れる時。派手さは減るが、深みと成熟が増す。教える・伝える役割に向く段階。",
    "「実力者から賢者へ」の移行段階。攻めの姿勢を緩め、守りと伝承に重心を移す時期。後進への教え・自分の智慧の体系化に取り組むと、人生の意味が深まる。",
  ],
  病: [
    "立ち止まり内省する段階。健康・人間関係・仕事に陰りを感じやすい時期だが、それは魂が休息と再考を要求しているサイン。芸術・学問・スピリチュアルに親和。",
    "「病は気づきの贈り物」のフェーズ。表面的には停滞や不調が現れやすいが、それは過剰だった生き方への警鐘。立ち止まり、自分の本当の声に耳を澄ます貴重な期間。",
  ],
  死: [
    "終焉と再構築への準備の段階。古いものが終わり、新しい何かが宿るための空白期。哲学的・霊的な深まりがあり、転機の年に当たることが多い。",
    "「魂の浄化」の段階。物理的な死ではなく、古い役割・古い人間関係・古い自我の死。それを受け入れることで、次のステージに向けた本当の自由が手に入る転機の時期。",
  ],
  墓: [
    "蓄積と内なる充実の段階。地味で動きの少ない時期だが、内面では深い知恵と財が蓄えられる。研究・学問・遺産・先祖供養に縁が深い段階。",
    "「沈黙の蔵」の段階。表に出ない代わりに、内側に深い宝が蓄えられる時期。研究者・職人・遺産管理人のような「奥に潜むエネルギー」が最大限に発揮される。",
  ],
  絶: [
    "切り替わりの段階。古い枠が完全に外れ、新しい流れが始まる前の真空状態。不安定だが自由、何でも始められる白紙状態。リセットとリスタートの段階。",
    "「ゼロからの再生」の段階。過去のしがらみが完全に解け、新しい自分として再起動するための真空地帯。一見ハイリスクだが、最も大きな変革が起こせる可能性を秘めた時期。",
  ],
  胎: [
    "新たな構想が宿る段階。まだ見えないが、内側で新しい命・プロジェクト・関係が育ち始めている時期。表に出る前の準備期間、可能性に満ちた静寂の時。",
    "「内なる芽生え」の段階。世界からは何も起きていないように見えるが、子宮の中で新しい命が育つように、見えないところで未来が静かに準備されている貴重な時期。",
  ],
  養: [
    "じっくり育てる段階。胎の段階で宿ったものを、丁寧に養い育てる時期。穏やかで安全な環境のなかで、未来の力が静かに蓄えられる。",
    "「保護された成長」の段階。激しい変化はないが、安全な環境で着実に育つフェーズ。焦らず、丁寧に積み重ねることで、次の長生での開花に向けた力が蓄えられる時期。",
  ],
};

export const TWELVE_TEXT: Record<TwelveStage, string> = Object.fromEntries(
  Object.entries(TWELVE_VARIANTS).map(([k, v]) => [k as TwelveStage, v[0]])
) as Record<TwelveStage, string>;

export function twelveText(stage: TwelveStage, date: Date = new Date()): string {
  const variants = TWELVE_VARIANTS[stage];
  if (!variants) return TWELVE_TEXT[stage] ?? "";
  const k = date.getFullYear() * 10000 + (date.getMonth() + 1) * 100 + date.getDate();
  return variants[k % variants.length];
}
